      ...
      /*  private async void Check()
        ...
        private async void скачатьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (var httpResponseMessage = await client.GetStreamAsync("api/user7/Download?fileName=4.zip"))
                {
                    httpResponseMessage.Read(READBUFFER_SIZE, 0, 1024);
                    using (FileStream FS = new FileStream("./Uploads/4.zip", FileMode.Create, FileAccess.Write, FileShare.Write))
                    {
                        long n = FS.Length;
